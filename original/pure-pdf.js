...
// ~264 pt panels; the address must land in the panel that faces the envelope
// window (the bottom panel, y 0-264).
//
// The safe zone is smaller than the window itself, because the folded piece
// floats inside the envelope: 0.458" of vertical slack (a 3.667" piece in a
// 4.125" envelope) and 1.0" horizontal. The address has to stay visible at
// EVERY position the piece can take, not just resting at the bottom, so these
// bounds are the intersection of the extremes rather than the aperture:
//
//   window aperture   1.125" x 4.5", inset 0.875" left / 0.5" bottom
//   minus the slack   ->  y 36-84 pt,  x 63-315 pt
//
...
//
// These are calibration values, not published specs: the FPi 700 manual was not
// available when they were derived, and window position varies by envelope
// manufacturer. Re-measure them per spec §8 against the actual machine and
// envelope stock before trusting a production run.
const WINDOW_Y_MIN = 36, WINDOW_Y_MAX = 84, WINDOW_X_MAX = 315;
